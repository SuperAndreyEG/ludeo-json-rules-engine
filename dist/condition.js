    ...
    value: function evaluate(almanac, operatorMap) {
      ...
      .then(function (rightHandSideValue) {
        return almanac.factValue(_this.fact, _this.params, _this.path).then(function (leftHandSideValue) {
          if (typeof _this.factModifier === 'function') {
            var state = almanac.getState();
            var stateValue = state[_this.fact];
            if (stateValue !== undefined) {
              var newValue = _this.factModifier(stateValue, leftHandSideValue);
              state[_this.fact] = newValue;
